...
def load_phase23_config(path: str | Path) -> Phase23Config:
...
def run_phase23_feasibility_ceiling_audit(
    config_path: str | Path = "configs/phase23_feasibility_ceiling_audit.yaml",
    output_dir: str | Path = "outputs/phase23_feasibility_ceiling_audit",
    *,
    progress: bool = False,
) -> dict[str, Path | pd.DataFrame]:
    config = load_phase23_config(config_path)
    ...
    for tier_idx, tier in enumerate(phase5_config.tiers):
        ...
        for scenario_id in range(scenario_count):
            ...
            if progress:
                print(
                    f"phase23_oracle tier={tier.name} scenario={scenario_id:03d}",
                    flush=True,
                )
    ...
    summary_path = output_path / "phase23_oracle_summary.csv"
    trajectories_path = output_path / "phase23_oracle_trajectories.csv"
    ceiling_path = output_path / "phase23_feasibility_ceiling.csv"
    comparison_path = output_path / "phase23_ceiling_comparison.csv"
    summary.to_csv(summary_path, index=False)
    trajectories.to_csv(trajectories_path, index=False)
    ceiling.to_csv(ceiling_path, index=False)
    comparison.to_csv(comparison_path, index=False)
    figure_paths = write_phase23_figures(
        summary=summary,
...
def solve_truth_consistent_slack_oracle(
    *,
    tier_name: str,
    ...
    truth_aero = perturb_aero(aero, scenario)
    actuator_tau = max(
        0.5 * settings.dt_s,
    ...
    trajectory = _truth_oracle_trajectory_frame(
        tier_name=tier_name,
        scenario=scenario,
        reference=horizon,
    ...
    metrics = {
        "controller": "truth_consistent_slack_oracle",
...
def compare_phase23_ceiling(
    *, config: Phase23Config, ceiling: pd.DataFrame
) -> pd.DataFrame:
    phase20 = pd.read_csv(config.phase20_output_dir / "phase20_ceiling_gap.csv")
    phase20_baseline = phase20[phase20["controller"].eq("ridge_safety")][
        [
            "tier",
            "strict_success_count",
            "controlled_recovery_count",
            "oracle_strict_feasible_count",
            "oracle_near_feasible_count",
        ]
    ].rename(
        columns={
            "strict_success_count": "phase20_online_success_count",
            "controlled_recovery_count": "phase20_controlled_count",
            "oracle_strict_feasible_count": "phase20_oracle_feasible_count",
            "oracle_near_feasible_count": "phase20_oracle_near_feasible_count",
        }
    )
    phase22 = pd.read_csv(config.phase22_output_dir / "phase22_variant_comparison.csv")
    phase22_best = (
        phase22.sort_values(["tier", "strict_success_count"], ascending=[True, False])
        ...
        .rename(
            columns={
                "controller": "phase22_best_controller",
                "strict_success_count": "phase22_best_success_count",
                "controlled_recovery_count": "phase22_best_controlled_count",
            }
        )
    )
    audited = ceiling.rename(
        columns={
            "feasible_count": "phase23_audited_feasible_count",
            "feasible_rate": "phase23_audited_feasible_rate",
            "near_feasible_count": "phase23_audited_near_feasible_count",
            "near_feasible_rate": "phase23_audited_near_feasible_rate",
        }
    )
    merged = audited.merge(phase20_baseline, on="tier", how="left").merge(
        phase22_best, on="tier", how="left"
    )
    merged["phase23_minus_phase20_oracle_count"] = (
        merged["phase23_audited_feasible_count"]
        - merged["phase20_oracle_feasible_count"]
    )
    merged["phase23_minus_phase22_online_count"] = (
        merged["phase23_audited_feasible_count"] - merged["phase22_best_success_count"]
    )
    return merged


def write_phase23_figures(
    *,
    ...
    paths = {
        "ceiling_png": output_dir / "audited_feasibility_ceiling.png",
        "comparison_png": output_dir / "phase23_ceiling_vs_online.png",
        "miss_png": output_dir / "audited_oracle_alpha_miss.png",
        "trajectory_png": output_dir / "audited_oracle_worst_cases.png",
    }
    _plot_ceiling(ceiling, paths["ceiling_png"])
    _plot_comparison(comparison, paths["comparison_png"])
    _plot_miss(summary, paths["miss_png"])
    _plot_worst_trajectories(summary, trajectories, paths["trajectory_png"])
    return paths


def _rk4_truth_oracle_symbolic(
    state: ca.MX,
    command: ca.MX,
    row: pd.Series,
    vehicle: VehicleParams,
    ...
    k1 = _truth_oracle_dynamics_symbolic(
        state, command, row, vehicle, aero, scenario, actuator_tau_s
    )
    k2 = _truth_oracle_dynamics_symbolic(
        state + 0.5 * dt * k1,
        command,
        row,
        vehicle,
    ...
    k3 = _truth_oracle_dynamics_symbolic(
        state + 0.5 * dt * k2,
        command,
        row,
        vehicle,
    ...
    k4 = _truth_oracle_dynamics_symbolic(
        state + dt * k3,
        command,
        row,
        vehicle,
...
def _truth_oracle_dynamics_symbolic(
    state: ca.MX,
    command: ca.MX,
    row: pd.Series,
    vehicle: VehicleParams,
...
def _truth_oracle_trajectory_frame(
    *,
    tier_name: str,
    scenario: UncertaintyScenario,
    reference: pd.DataFrame,
        ...
        rows.append(
            {
                ...
                * scenario.density_scale,
                ...
                "actuator_tau_prediction_s": actuator_tau,
                "density_scale": scenario.density_scale,
                "cm_alpha_scale": scenario.cm_alpha_scale,
                "cm_delta_scale": scenario.cm_delta_scale,
                "cm_q_scale": scenario.cm_q_scale,
                "external_disturbance_moment_nm": (
                    scenario.external_disturbance_moment_nm
                ),
...
def _plot_ceiling(ceiling: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_ylim(0.0, 1.0)
    ax.set_ylabel("audited oracle ceiling rate")
...
def _plot_comparison(comparison: pd.DataFrame, path: Path) -> None:
    ...
    ax.bar(
        x - 0.25,
        comparison["phase20_oracle_feasible_count"],
        width=0.25,
        label="Phase 20 oracle",
    )
    ax.bar(
        x,
        comparison["phase23_audited_feasible_count"],
        width=0.25,
        label="Phase 23 audited oracle",
    )
    ax.bar(
        x + 0.25,
        comparison["phase22_best_success_count"],
        width=0.25,
        label="Phase 22 online",
    )
    ax.set_xticks(x, comparison["tier"])
    ax.set_ylabel("strict success / feasible count")
    ax.grid(axis="y", alpha=0.25)
...
def _plot_miss(summary: pd.DataFrame, path: Path) -> None:
    ...
    ax.axhline(0.001, color="black", linewidth=0.8, linestyle="--")
    ax.axhline(0.025, color="gray", linewidth=0.8, linestyle=":")
    ax.set_ylabel("audited oracle max alpha miss [rad]")
    ax.grid(axis="y", alpha=0.25)
    fig.tight_layout()
    fig.savefig(path, dpi=180)
    plt.close(fig)
